...
# .NET Hosting Bundle
Install-Software -Name ".NET 6.0 Hosting Bundle" -FilePath ".\dotnet-hosting-6.0.15\dotnet-hosting-6.0.15-win.exe" -Arguments "/install /quiet /norestart"

# SQL Server Compact Edition (x86)
Install-Software -Name "SQL Server Compact Edition (x86)" -FilePath ".\sql server compact edition\SSCERuntime_x86-DEU.msi" -Arguments "/i `"#FilePath#`" /quiet /norestart" -Installer "msiexec.exe"

# SQL Server Compact Edition (x64)
Install-Software -Name "SQL Server Compact Edition (x64)" -FilePath ".\sql server compact edition\SSCERuntime_x64-DEU.msi" -Arguments "/i `"#FilePath#`" /quiet /norestart" -Installer "msiexec.exe"

# .NET Framework 4.8
Install-Software -Name ".NET Framework 4.8" -FilePath ".\dotnetfx48\ndp48-x86-x64-allos-enu.exe" -Arguments "/q /norestart"

# Visual C++ Redistributable 2019 (x64)
Install-Software -Name "Visual C++ Redistributable 2019 (x64)" -FilePath ".\vcredist\VC_redist.x64.exe" -Arguments "/install /quiet /norestart"

# Visual C++ Redistributable 2019 (x86)
Install-Software -Name "Visual C++ Redistributable 2019 (x86)" -FilePath ".\vcredist\VC_redist.x86.exe" -Arguments "/install /quiet /norestart"

# SQL Server OLE DB Driver
Install-Software -Name "SQL Server OLE DB Driver" -FilePath ".\sqlServerOleDB\msoledbsql.msi" -Arguments "/i `"#FilePath#`" /quiet /norestart IACCEPTMSOLEDBSQLLICENSETERMS=`"YES`"" -Installer "msiexec.exe"

# Nova Setup
# Install-Software -Name "Nova Setup" -FilePath ".\data\NovaSetup_x64.msi" -Arguments "/i `"#FilePath#`" /quiet /norestart" -Installer "msiexec.exe"
Install-Software -Name "Nova Setup" -FilePath ".\data\NovaSetup_x64.msi" -Arguments "/i `"#FilePath#`" /quiet /norestart /l*v nova_install.log" -Installer "msiexec.exe"

# Nova Patch
Install-Software -Name "Nova Patch" -FilePath ".\data\NovaPatch_x64.msp" -Arguments "/p `"#FilePath#`" /quiet /norestart REINSTALLMODE=`"omus`" REINSTALL=`"ALL`"" -Installer "msiexec.exe"

Show-Banner "Installation Process Completed"
Write-Host "All software packages have been installed. Please check the output for any errors." -ForegroundColor Green
