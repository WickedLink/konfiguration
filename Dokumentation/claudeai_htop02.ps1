function Show-ProcessMonitor {
    ...
    while ($true) {
        ...
        Write-Host "Press: Q to quit, K to kill, C (CPU), M (Memory), N (Name) to sort, D to change display count, L to filter" -ForegroundColor Yellow
        ...
        switch ($key.Character) {
            ...
            'l' {
                $newFilter = Read-Host "Enter the filter (e.g., 'n*' for processes starting with 'n', '*' for all)"
                if ($newFilter) {
                    $Filter = $newFilter
                }
                else {
